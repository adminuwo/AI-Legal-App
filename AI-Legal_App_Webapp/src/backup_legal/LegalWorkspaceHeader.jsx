    ...
    <div className="w-full px-4 sm:px-12 pt-3 sm:pt-4 mb-4 sm:mb-8 bg-transparent">
      <div className="flex flex-col gap-4 sm:gap-6">
        ...
        <div className="flex items-center justify-between w-full">
          {selectedLegalTool?.id === 'legal_my_case' && currentCase && (
            <button
              onClick={handleBackToDashboard}
              className="flex items-center gap-2 text-slate-500 hover:text-indigo-600 font-bold text-[9px] sm:text-[10px] uppercase tracking-widest bg-slate-100 ] px-3 py-1.5 sm:px-4 sm:py-2 rounded-full transition-all hover:gap-3 group border border-transparent"
            >
        ...
        {currentCase && (
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 sm:gap-6 p-3 sm:p-0 rounded-2xl sm:rounded-none border border-slate-100 sm:border-none w-full">
            <div className="flex items-center gap-3 sm:gap-5 w-full sm:flex-1 min-w-0">
              ...
              <div className="relative group shrink-0 hidden xs:block">
                <div className="relative p-2 sm:p-4 rounded-xl sm:rounded-2xl text-white shadow-sm sm:shadow-sm shadow-indigo-500/10">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                   {isRenamingCase === currentCase._id ? (
                    <input
                      ...
                      className="bg-slate-50 border border-indigo-500 rounded-lg px-2 py-0.5 text-xs sm:text-lg font-black w-full outline-none text-slate-900"
                      autoFocus
                    />
                  ) : (
                    <div className="px-2 py-0.5 bg-indigo-50 ] rounded-lg border border-indigo-100">
                      <h2 className="text-[12px] sm:text-2xl font-black text-indigo-700 ] tracking-tight leading-tight uppercase truncate">
                        {currentCase.name}
                      </h2>
                    </div>
                  )}
                ...
                <div className="flex items-center gap-x-2 sm:gap-x-4">
                  <p className="text-[8px] sm:text-xs text-indigo-600 font-black uppercase tracking-widest flex items-center gap-1">
            ...
            {/* Tool Identity Row */}
            <div className="flex items-center justify-between sm:justify-end w-full sm:w-auto gap-2 sm:gap-3 shrink-0 mt-1 sm:mt-0 pt-3 sm:pt-0 border-t border-slate-200 sm:border-none">
                <div className="flex flex-col items-start sm:items-end">
                   ...
                   <AnimatePresence mode="wait">
                    <motion.div
                      ...
                      exit={{ y: -5, opacity: 0 }}
                       className="flex items-center gap-1 px-1.5 py-0.5 sm:px-4 sm:py-2 bg-slate-100 ] text-slate-500 ] border border-slate-200 rounded-md sm:rounded-full"
                    >
                ...
                <div className="flex items-center gap-1 sm:gap-2">
                   <button
                    ...
                    className="p-2 sm:p-2.5 bg-white ] border border-slate-200 text-slate-400 hover:text-indigo-600 rounded-lg sm:rounded-full transition-all shadow-sm group"
                    title="Rename Case"
                  >
                   ...
                   <button
                    ...
                    className="p-2 sm:p-2.5 bg-white ] border border-slate-200 text-slate-400 hover:text-red-500 rounded-lg sm:rounded-full transition-all shadow-sm group"
                    title="Delete Case"
                  >
        ...
        {/* My Case Internal Navigation */}
        {currentCase && (
          <div className="w-full overflow-x-auto custom-scrollbar border-b border-slate-200 pb-0.5">
            <div className="flex items-center gap-6 min-w-max px-2">
              {[
                'Overview', 'Timeline', 'Parties', 'Hearings', 'Documents', 
                'Evidence', 'Research', 'Drafts', 'Arguments', 'Strategy', 
                'Prediction', 'Contracts', 'Notes', 'Tasks', 'AI Chat', 
